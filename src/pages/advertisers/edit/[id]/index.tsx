...
import { getAdvertiserById, updateAdvertiserById } from 'apiSdk/advertisers';
...
function AdvertiserEditPage() {
  ...
  const handleSubmit = async (values: AdvertiserInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAdvertiserById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<AdvertiserInterface>({
    initialValues: data,
