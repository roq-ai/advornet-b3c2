...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['sales-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AdvertiserInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/advertisers` : null),
    () => getAdvertisers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Advertiser',
      path: isTenantUser ? `/advertisers/view/${data?.[0]?.id}` : '/advertisers',
      entity: 'advertiser',
